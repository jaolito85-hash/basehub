...
import { MAX_MEDIA_PER_PRODUCT, isMediaFile } from '@/lib/products/media'
...
export default function NovoProdutoPage() {
  ...
  function handleAddContent() {
    ...
    if (isMedia) {
      const currentMediaCount = contents.filter((c) => c.uploadType === 'media').length
      ...
      if (!isMediaFile(uploadFile)) {
        setError('Para midia, envie apenas arquivos de imagem (JPG, PNG, WebP, GIF, SVG).')
        return
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (imageFile) {
      ...
      if (uploadError) {
        setError('Erro ao fazer upload da imagem. Verifique se o bucket "product-images" existe no Supabase.')
        setLoading(false)
        return
      }
    ...
    if (insertError || !productData) {
      setError('Erro ao cadastrar o produto: ' + (insertError?.message || 'Desconhecido'))
      setLoading(false)
      return
    }
    ...
    for (const content of contents) {
      ...
      if (content.uploadType === 'media') {
        const { error: insertMediaError } = await supabase.from('videos').insert({
          product_id: productId,
          title: content.title,
          description: content.description || null,
          type: content.category,
        ...
        if (insertMediaError) {
          setError(`Erro ao registrar midia: ${content.title}`)
          setLoading(false)
          return
        }
      } else if (content.uploadType === 'video') {
  ...
  return (
    <div className="max-w-xl space-y-6">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold text-text-primary">Novo Produto</h1>
        <p className="text-sm text-text-muted mt-0.5">Cadastre um produto para organizar vídeos e manuais</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl shadow-card p-6 space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Ex: Cadeira ergonômica com apoio lombar ajustável, altura regulável 42–52cm, suporta até 150kg, base em alumínio..."
            rows={4}
            className="
          ...
          <p className="text-[11px] text-text-muted mt-1">
            Aparece na página do produto e ajuda as equipes de vendas a entender o item.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-border">
          <div className="flex flex-col gap-3 mb-4">
            <h3 className="text-sm font-bold text-text-primary">Conteúdos do Produto</h3>
            <p className="text-xs text-text-muted">Adicione vídeos, arquivos e mídias que farão parte deste produto.</p>
          </div>
          ...
          {contents.length > 0 && (
            <div className="space-y-2 mb-4">
              {contents.map((c) => (
                <div key={c.id} className="flex items-center justify-between p-3 bg-canvas border border-border rounded-lg">
        ...
        {success && (
          <div className="flex items-center gap-2 p-3 rounded-lg bg-success/10 border border-success/20">
            ...
            <p className="text-xs text-success">Produto cadastrado com sucesso! Redirecionando...</p>
          </div>
        )}
